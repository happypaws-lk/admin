...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <motion.aside
        initial={false}
        animate={{ x: !isMobile || isMobileOpen ? 0 : "-100%" }}
        transition={{ type: "spring", bounce: 0, duration: 0.4 }}
        className="fixed inset-y-0 left-0 z-40 w-64 flex-shrink-0 h-screen flex flex-col apple-glass-sidebar md:relative"
      >
        {/* Brand Header */}
          ...
          <div>
            ...
            <div className="space-y-1">
              {DASHBOARD_ITEMS.map((item) => (
                <NavItem key={item.href} {...item} onClick={() => isMobile && setIsMobileOpen(false)} />
              ))}
            </div>
          </div>
